  ...
  return (
    <section ref={sectionRef} className={`${styles.root} w-full overflow-hidden`}>
      <div className="px-50 md:px-100 py-100">
        ...
        <div className={`${styles.timeline} relative block landscape:flex md:flex flex-nowrap items-stretch w-full my-200`}>

          {block?.timeline?.map((block: any, index: any) => {
            //console.log('col: ', index, block)
            return (
              <div className={`${styles.item} item relative flex-1 mb-10 landscape:mb-0 md:mb-0`} key={index}>
                {block.image && (
                  <div className={`${styles.top} absolute w-full h-150 bottom-0`}>
                    <div className='absolute w-full h-full top-0 left-0 overflow-hidden'>
                      <Image
                        ...
                        className={`${styles.image} relative w-full h-full landscape:h-auto md:h-auto object-cover`}
                      />
                    </div>
                  </div>
                )}
                <div className={`${styles.mid} relative bg-orange z-10`} style={{ backgroundColor: block.color }}>
                  <div className='flex items-stretch px-10 py-20 landscape:py-10 md:py-10'>
